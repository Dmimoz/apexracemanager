...
function buildTrack(c: Circuit): TrackGeo {
  ...
  // Вытянутые реальные контуры (Монца, Баку) выглядели бы тонкой нитью —
  // мягко растягиваем короткую ось (до ×1.35), чтобы трасса читалась на экране.
  // Движение болидов идёт по этой же линии, так что на симуляцию не влияет.
  const aspect = wUsed / (hUsed || 1);
  const stretchY = aspect > 2.1 ? Math.min(aspect / 2.1, 1.35) : 1;
  for (const p of pts) {
